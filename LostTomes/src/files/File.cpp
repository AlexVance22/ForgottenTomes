...
void File::save(const fs::path& filepath)
{
	this->filepath = filepath;

	std::ofstream stream(filepath, std::ofstream::trunc);
	if (!stream.is_open())
		throw FileNotFound("cannot open file for save");
	...
	rootdir = filepath.stem();
}

void File::load(const fs::path& filepath)
{
	this->filepath = filepath;

	std::ifstream stream(filepath);
	if (!stream.is_open())
		throw FileNotFound("cannot open file for load");
